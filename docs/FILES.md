...
| `test/` | `leak-check.js` (CI gate, needs model), `rehydrate.test.js` (no model), `leak-stats.mjs` (on-demand recall/precision benchmark over the ai4privacy dataset, needs model — `pnpm run leak-stats`). Imports from `../src/`. |
