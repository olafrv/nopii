...
| `test/` | `leak-check.js` (CI gate, needs model), `rehydrate.test.js` (no model), `leak-stats.mjs` (on-demand recall/precision benchmark over the ai4privacy dataset, needs model — `pnpm run leak-stats`; guide in `test/LEAK_TEST.md`). Imports from `../src/`. |
...
| `scripts/download-dataset.mjs` | Fetches an ai4privacy PII split from Hugging Face into `datasets/` (mirrors the HF repo path), for the leak-stats benchmark. Run via `pnpm run dataset:download`. See `test/LEAK_TEST.md`. |
